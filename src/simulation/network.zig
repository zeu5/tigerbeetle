...
pub const NetworkOptions = struct { node_count: u8, client_count: u8, seed: u64, recorded_count_max: usize, path_maximum_capacity: usize };
pub const Path = struct {
    source: Process,
    target: Process,
};

pub fn NetworkType(comptime NetworkSimulatorType: anytype) type {
    const MessageBus = MessageBusType(NetworkSimulatorType);
    ...
    return struct {
        ...
        network_simulator: NetworkSimulator(Packet, NetworkSimulatorType),
        ...
        pub fn init(
            ...
            var network_simulator = try NetworkSimulator(Packet, NetworkSimulatorType).init(allocator);
            errdefer network_simulator.deinit();
